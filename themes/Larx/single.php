...
<section class="i-blog-section-inner">
	...
	<?php
       ...
       <section id="services" class="vc_row-fluid text-center container ">	
      <div class=" ">
	...
	global $post;
	$parent = $post->ID;
	$children = get_posts('post_parent='.$parent.'&&post_type=services&showposts=20&post_status=publish');
		...
		$my_postid = $childID;//This is page id or post id
		...
		$content = str_replace(']]>', ']]&gt;', $content);?>
            ...
            <div class="col-md-4 vc_custom">
              
        <div class="service-box">
                <div class=""> <a href="<?php echo get_the_permalink($my_postid);?>"><i class="fa fa-picture-o fa-4x"></i></a>
        <div class="service-title">
        
                    <h3> <a href="<?php echo get_the_permalink($my_postid);?>"><?php echo $child_post_title=$child->post_title;?></a> </h3>
                  </div> 	
		...
		<?php echo '<p class="servicecon">'.$child->post_excerpt.'<br/>       
        <a href="'.get_the_permalink($my_postid).'"> Read More </a></p>'; 
		?>
        </div></div></div>
...
?></div></div></div></div></div></section>
							...
							<?php if (th_theme_data('switch_next_prev') == 1){ ?>
								<div class="pager">
									<?php
									//echo previous_post_link('%link','<button type="button" class="btn gold-btn" style="float:left"><i class="fa fa-arrow-left"></i> '.esc_html('Older', 'larx').'</button>');
									//echo next_post_link('%link','<button type="button" class="btn gold-btn" style="float:right">'.esc_html('Newer', 'larx').' <i class="fa fa-arrow-right"></i></button>');
									?>
								</div>
							<?php } ?><?php */?>
