...
function th_wp_title( $title, $sep ) {
	global $paged, $page;
	...
	if ( $paged >= 2 || $page >= 2 )
		$title = "$title $sep " . sprintf( __( 'Page %s', 'larx' ), max( $paged, $page ) );

	return $title;
}
add_filter( 'wp_title', 'th_wp_title', 10, 2 );

//Custom post tags
function th_blog_post_tags(){
	if(has_tag()){
	?>
	<p class="post-tags"><?php //the_tags('<span>'.esc_html_e('Tags', 'larx').': </span>', ', ', '<br />'); ?></p>
	<?php
	}
}
...
function th_comments_script() {
	if(is_singular())
	wp_enqueue_script('comment-reply');
}
add_action('wp_enqueue_scripts', 'th_comments_script');